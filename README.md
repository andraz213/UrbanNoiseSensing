...
Another activity is time synchronization. The sensor asks gateway what time in milliseconds it is, so it can put a timestamp on the sensed data when sending it and to start collectiong voice samples at the same time as all the other sensors. 
...
Gateway will be used to recieve data from the sensors and send it to the backednd. 
...
The first one will handle all the data coming from the sensors and the time sychronisation. The second one will take the reciecved data, package it into json format and send it to backend.

These two ESP32s will communicate over UART connection with a special PJON protocol. https://www.pjon.org/ThroughSerial.php
